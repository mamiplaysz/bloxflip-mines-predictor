...
async def mines(ctx, round_id):
    ...
    if round_length < 36:
        ...
        pfp = 'https://tenor.com/view/yh-gif-25406060'
        em = discord.Embed(color=0x11F1D3)
        em.set_thumbnail(url=pfp)
        em.set_footer(text="Made by 7LUX#0666")
        em.add_field(name="Mines predictor",value=row1 + "\n" + row2 + "\n" + row3 + "\n" + row4 +"\n" + row5 + "\n" + "**Accuracy**" + "\n" + info +"%")
        await ctx.reply(embed=em)


bot.run('MTAzOTIyMzM0NTgzNDYzNTI3NA.GFklU1.JJM5s5hfgUcG-f4sYOhQ3ZN43u819fr8m61ws8')
